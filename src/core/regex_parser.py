    ...
    while i < len(regex):
        ...
        if c in {"*", "+", "?", "|", "(", ")"}:
            ...
            while j < len(regex) and regex[j] != "'":
                j += 1
            if j == len(regex):
                raise ValueError(f"Aspas simples não fechadas: {regex}")
    ...
    for i in range(len(tokens)):
        ...
        if i + 1 < len(tokens):
            ...
            if curr_type in {
                ...
                RegexToken.PLUS,
                RegexToken.QUESTION,
            } and next_type in {RegexToken.CHAR, RegexToken.LPAREN}:
...
def get_regex_from_lines(lines: list[str]) -> list[TokenType]:
    ...
    for i, (categoria, regex) in enumerate(regex_list):
        ...
        tokentype = TokenType(
            name=categoria, regex=tokens_without_concat, dfa=None
        )  # DFA será construído posteriormente
...
def get_regex_from_file(file_path: str) -> list[TokenType]:
    ...
    if not lines:
        raise ValueError(
            f"O arquivo {file_path} está vazio ou não contém uma regex válida."
        )
    return get_regex_from_lines(lines)


def resolve_references_add_concats(
    tokentypes_without_concat: list[TokenType],
) -> list[TokenType]:
    token_map = {t.name: t for t in tokentypes_without_concat}

    def expand_token_list(
        tokens: list[RegexToken], visited: set[str]
    ) -> list[RegexToken]:
        resolved = []
        for token in tokens:
            if token.type == RegexToken.REF:
                ...
                if ref_name in visited:
                    raise ValueError(
                        f"Referência cíclica detectada: {' → '.join(visited)} → {ref_name}"
                    )

                # Expande recursivamente a referência
                sub_tokens = expand_token_list(
                    token_map[ref_name].regex, visited | {ref_name}
                )
